...
PARAMS = {
    "antro":    {"beta0": 0.25, "beta1": 1.0},
    "Standford":{"beta0": 0.05, "beta1": 0.65},
    "DLR":      {"beta0": 0.00, "beta1": 0.50}
}
...
def qpso_solve(DH, TARGET, beta0, beta1, particles=n_particles,
...
def main():
    for robot in robots:
        ...
        for mode in modes:
            ...
            LOGfile = f"QPSO/log_{robot}_{mode}.csv"
            ...
            for i, Tt in enumerate(Tlist, 1):
                qf, conv, pos, ori, its, time = qpso_solve(DH, Tt, beta0, beta1)
                J = JAC(DH, qf)
                mu, kappa = manip(J)
