...
PARAMS = {
    "antro":    {"c1": 1.88, "c2": 0.49, "w": 0.56},
    "Standford":{"c1": 1.8875, "c2": 1.6425, "w": 0.345},
    "DLR":      {"c1": 1.44, "c2": 1.43, "w": 0.5}
}
...
def log_row(path, conv, pos, ori, its, elapsed, mu, kapp, vars):
    ...
    with open(path, 'a', newline='') as f:
        ...
        if head:
            ...
            w.writerow(h)
        w.writerow([int(conv), float(pos), float(ori), int(its), float(elapsed), float(mu), float(kapp)] + [float(x) for x in vars])
...
def pso_solve(DH, TARGET, c1, c2, w, particles=n_particles,
              max_it=MAX_IT, tol_pos=TOL_POS, tol_ori=TOL_ORI):
    n = DH.shape[0]
    rng = np.random.default_rng()
    limits = np.asarray(infer_limits(DH))
    lo = limits[:,0]; hi = limits[:,1]
    m = particles
    _ = cost(DH, np.zeros((1, n)), TARGET)
    t0 = time.perf_counter()
    pos = rng.uniform(lo, hi, size=(m, n))
...
def main():
    for robot in robots:
        ...
        DH = load_csv(DHfile)
        params = PARAMS[robot]
        c1 = params["c1"]; c2 = params["c2"]; w = params["w"]
        for mode in modes:
            TARGETfile = f"TARGETS/TARGET_{robot}_{mode}.csv"
            LOGfile = f"PSO/log_{robot}_{mode}.csv"
            ...
            for i, Tt in enumerate(Tlist, 1):
                qf, conv, pos, ori, its, time = pso_solve(DH, Tt, c1, c2, w)
                J = JAC(DH, qf)
                mu, kappa = manip(J)
                dvs = dq_variations(DH, q0, qf)
                log_row(LOGfile, conv, pos, ori, its, time, mu, kappa, dvs)
